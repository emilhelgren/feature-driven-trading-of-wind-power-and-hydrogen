...
function get_forecasted_plan(training_period_length, bidding_start)
    ...
    for day in days
        ...
        for t in day
            ...
            if lambda_F[t+offset] < lambda_H
                @constraint(initial_plan, forward_bid[t] == sum(qF[i, 1] * x_max[t+offset][i] for i in 1:2) + qF[3, 1])
                @constraint(initial_plan, hydrogen[t] == sum(qH[i, 1] * x_max[t+offset][i] for i in 1:2) + qH[3, 1])
            elseif lambda_F[t+offset] < top_domain
                @constraint(initial_plan, forward_bid[t] == sum(qF[i, 2] * x_max[t+offset][i] for i in 1:2) + qF[3, 2])
                @constraint(initial_plan, hydrogen[t] == sum(qH[i, 2] * x_max[t+offset][i] for i in 1:2) + qH[3, 2])
            else
                @constraint(initial_plan, forward_bid[t] == sum(qF[i, 3] * x_max[t+offset][i] for i in 1:2) + qF[3, 3])
                @constraint(initial_plan, hydrogen[t] == sum(qH[i, 3] * x_max[t+offset][i] for i in 1:2) + qH[3, 3])
            end
        end
    end
...
for i in 1:12
    ...
    for v in 1:(validation_period+test_period)
        ...
        if (v % (720) == 0)
            ...
            filename = "2020_red_elec/changing_qs_simple/$(n_months)_month_training/simple_pricedomain_RETRAINING_mo$(Int(round(v/720)))"
...
#             filename = "2020_red_elec/changing_qs_simple/$(n_months)_month_training/simple_forecasted_model_pricedomain_RETRAINING_mo$(Int(round(v/720)))"
#             easy_export(data, names, filename,)
...
#             filename = "2020_red_elec/changing_qs_simple/$(n_months)_month_training/simple_fp_pricedomain_RETRAINING_mo$(Int(round(v/720)))"
#             easy_export(data, names, filename,)
