...
function get_initial_plan(training_period_length, bidding_start)
    ...
    for day in days
        @constraint(initial_plan, sum(hydrogen[t] + hydrogen_extra[t] for t in day) >= min_production)
        for t in day
            index = mod(t, 24)
            if (index == 0)
                index = 24
...
#     filename = "2022/pricedomains/medium_PRICEDOMAIN_PEN_mo$n_months"
...
#     filename = "2022/pricedomains/medium_forecast_model_PRICEDOMAIN_PEN_mo$n_months"
#     easy_export(data, names, filename,)
# end
...
for i in 1:3
    ...
    filename = "2022/pricedomains/medium_fp_PRICEDOMAIN_PEN_mo$n_months"
    easy_export(data, names, filename,)
end
